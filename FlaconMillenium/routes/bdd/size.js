...
Router.get('/list', async (req, res) => {
  try {
    if (req.user) {
      SIZE.find({}, function (error, result) {
        if (error) {
          // console.log('The size did not exist.')
          notFound(req, res, 0)
        } else {
          ...
          res.status(200).render('size/sizeList', { data: result, name: req.user.username, alert: alert })
  ...
  try {
    if (req.user) {
      if (typeof req.query.id === 'undefined') {
        ...
        SIZE.findById({ _id: req.query.id }, function (error, result) {
          if (error) {
            // console.log('The size did not exist.')
            notFound(req, res, 1)
          } else {
            if (result) {
              res.format({
                'text/html': function () {
                  res.status(200).render('size/sizeView', { data: result, name: req.user.username })
                },
...
Router.patch('/unit', async (req, res) => {
  try {
    if (req.user) {
      SIZE.findByIdAndUpdate({ _id: req.body.id }, {
        $set: {
          name: req.body.name,
          size: req.body.size,
          description: req.body.description
...
Router.delete('/unit', async (req, res) => {
  try {
    if (req.user) {
      SIZE.findByIdAndDelete({ _id: req.body.id }, function (error, result) {
        if (error) {
          // console.log('The size did not exist.')
          notFound(req, res, 1)
        } else {
...
function notFound (req, res, alert) {
  res.format({
    'text/html': function () {
      if (alert) {
        req.session.alert = 'Error : The size was not found.'
      }
      res.status(404).redirect('/home')
    },

    'application/json': function () {
      res.status(404).send('Error : The size was not found.')
    },
